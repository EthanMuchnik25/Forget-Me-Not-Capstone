...
with open('F:\Data\\Intermediary\\dataset.yaml', 'r') as stream:
    try:
...
def downLoadData():
    ...
    name = "coco-2017"
    dataset = foz.load_zoo_dataset(name, dataset_dir="F:\Data\initial-download", label_types="detections", split="validation")
    # session = fo.launch_app(dataset)

    dataset.export(
        export_dir="F:\Data\\Intermediary",               # Specify export path on the external drive
...
def addDatasetToBiggerDataset(datasetPath):
    ...
    for i in range(len(paths)):
        for imageLabel in imagesLabels:
            imageLabelPath = paths[i] + "\\" + imageLabel
...
def train():
    ...
    model = YOLO('YoloModels/yolov10m.pt')

    # Perform object detection
    model.train(data="F:\Data\data.yaml", epochs=25, imgsz=640, batch=8)
...
if __name__ == "__main__":
    ...
    if args.copyFiles:
        CopyFiles()
    if args.add:
        addDatasetToBiggerDataset(args.add)
    if args.train:
        train()
